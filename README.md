...
## Онлайн-версия
Онлайн-версия проекта расположена по следующему [адресу](https://wannabenormal.ru).

## Docker-режим
Проект поддерживает `dev` и `prod`-режимы с помощью `Docker`.
Для того чтобы запустить `dev`-режим необходимо следующее:

1. Определить переменную окружения `POSTGRES_HOST` как `db`.
2. Выполнить команду:
```sh
...
```
4. Сайт будет доступен по адресу: [http://localhost:8080](https://localhost:8080)
